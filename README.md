...
Sync Module 2 and you reboot it, it can power on but never finish booting —
no activity, keyboard unresponsive, requiring a long power-button hold to
...
this use case — Watchman's clips are small and ingest isn't time-sensitive —
though 480Mbps ("Hi-Speed") isolator models exist at a similar price if you'd
...
- These need a passwordless sudo rule for `SERVICE_USER`, scoped to exactly
  the four `systemctl restart` commands for Watchman's own services —
  `setup.sh` installs this automatically as
